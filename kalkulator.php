...
$kolicina = isset($_GET['kolicina']) ? floatval($_GET['kolicina']) : 0;// uzima njegovu vrednost i konvertuje je u decimalni broj (float) pomoću floatval()
$gramPoUnci = 31.1035;
$kolicinaUnce = $kolicina / $gramPoUnci;
$trenutnaCena = $goldEur * $kolicinaUnce;
...
if (isset($data7['rates']['XAU']) && isset($data7['rates']['EUR'])) {
    ...
    $staraCena = $goldEur7 * $kolicinaUnce;
} else {
    $staraCena = null;
}

if ($staraCena !== null) { //Ako smo uspeli da dobijemo staru cenu iz API-ja (pre 7 dana), onda možemo da računamo
    $razlika = abs($trenutnaCena - $staraCena); // uvek pozitivna promena
    $projekcija = $trenutnaCena + $razlika;
    $profit = $trenutnaCena - $staraCena;
} else {
    $projekcija = null;
    $profit = null;
}

if (isset($_GET['akcija']) && $_GET['akcija'] === 'pdf') { //Ako "akcija" postoji, proverava da li je tačno string "pdf"
    ...
    $ime = isset($_SESSION['first_name']) ? $_SESSION['first_name'] : '';
    $prezime = isset($_SESSION['last_name']) ? $_SESSION['last_name'] : '';
    $korisnik = trim("$ime $prezime");

    $html = "<h1>Izveštaj o zlatu za korisnika: <b>$korisnik</b></h1>";
    $html .= "<p>Kolicina: {$kolicina} g</p>";
    $html .= "<p>Cena po unci: " . number_format($goldEur, 2) . " EUR</p>"; //Formatira broj tako da ima tačno 2 decimale
    $html .= "<p>Ukupno danas: " . number_format($trenutnaCena, 2) . " EUR</p>";
    $html .= "<p>Pre 7 dana: " . ($staraCena !== null ? number_format($staraCena, 2) . " EUR" : "N/A") . "</p>";
    $html .= "<p>Projekcija za 7 dana: " . ($projekcija !== null ? number_format($projekcija, 2) . " EUR" : "N/A") . "</p>";
...
<body>
  ...
  <main>
    ...
    <form>
      <label for="kolicina">Unesite količinu zlata (g):</label>
      <input type="number" id="kolicina" name="kolicina" placeholder="npr. 10" required>
    

    <div class="rezultat">
      <h2>Rezultati</h2>
      <ul>
        <li>Uneta količina: <?php echo $kolicina; ?> g</li>
        <li>Trenutna cena po unci: <?php echo number_format($goldEur, 2); ?> EUR</li>
        <li>Ukupno danas: <?php echo number_format($trenutnaCena, 2); ?> EUR</li>
        <li>Pre 7 dana: 
          <?php echo $staraCena !== null ? number_format($staraCena, 2) . " EUR" : "N/A"; ?>
        </li>
        <li>Za 7 dana (projekcija): 
          <?php echo $projekcija !== null ? number_format($projekcija, 2) . " EUR" : "N/A"; ?>
        </li>
        <li>Profit: 
          <?php echo $profit !== null ? number_format($profit, 2) . " EUR" : "N/A"; ?>
        </li>
      </ul>
      <button type="submit" name="akcija" value="izracunaj">Izračunaj</button>
      <button type="submit" name="akcija" value="pdf">Exportuj u PDF</button>
